...
class Merchant extends Base{
    ...
    public function newad(){
        ...
        if(request()->isPost()){
            ...
            if($_POST['zfb'] && !$iszfb){
                 $this->error('请先设置您的支付宝支付信息');
            }
            if($_POST['wx'] && !$iswx){
                 $this->error('请先设置您的微信支付信息');
            }
            // dump($isbank['name']);die;

            $ad_no = $this->getadvno();
            $model2 = new AdModel();
            $flag = $model2->insertOne(['userid'=>session('uid'),'add_time' => time(),'coin'=>'usdt','min_limit'=>$min_limit,'max_limit'=>$max_limit,'state'=>0,'pay_method'=>$_POST['bank'],'pay_method2'=>$_POST['zfb'],'pay_method3'=>$_POST['wx'],'ad_no'=>$ad_no,'amount'=>$amount,'price'=>$price,'state'=>1]);
            //增加在售广告数
            $count = $model2->where('userid', session('uid'))->where('state', 1)->where('amount', 'gt', 0)->count();
            $model->updateOne(['id'=>session('uid'), 'ad_on_sell'=>$count]);
            if($flag['code'] == 1){
	...
	public function newadbuy(){
        ...
        if(request()->isPost()){
            ...
            $ad_no = $this->getadvno();
            $model2 = new AdbuyModel();
            $flag = $model2->insertOne(['userid'=>session('uid'),'add_time' => time(),'coin'=>'usdt','min_limit'=>$min_limit,'max_limit'=>$max_limit,'pay_method'=>$_POST['bank'],'pay_method2'=>$_POST['zfb'],'pay_method3'=>$_POST['wx'],'ad_no'=>$ad_no,'amount'=>$amount,'price'=>$price,'state'=>1]);
            //增加求购广告数
            $count = $model2->where('userid', session('uid'))->where('state', 1)->where('amount', 'gt', 0)->count();
            $model->updateOne(['id'=>session('uid'), 'ad_on_buy'=>$count]);
			if($flag['code'] == 1){
  ...
  //       }
        ...
        if($type==0){
            ...
            // foreach($getpaymethod as $k=>$v){
                if($orderinfo['pay_method'] >0){
                 ...
                 if($orderinfo['pay_method2'] >0){
                    $zfb = $zfb->where('merchant_id', session('uid'))->find();
                    if(empty($zfb)){
                        $k2++;
                        $msg .= '支付宝信息未设置 ';
                    }
                 ...
                 if($orderinfo['pay_method3'] >0){
                     $wx = $wx->where('merchant_id', session('uid'))->find();
                    if(empty($wx)){
                        $k2++;
                        $msg .= '微信支付信息未设置 ';
                    }
            ...
            try{
                ...
                if ($rs1 && $rs2 && $rs3) {
                    Db::commit();
                 financelog(session('uid'),($num+$fee),'卖出USDT_冻结1',1);//添加日志
                    //todo:发送短信给买家
        ...
        if(request()->isPost()){
            ...
            try{
                ...
                if($rs1 && $rs2 && $rs3 && $rs4  && $rs6 && $rs7 && $rs8 && $rs9 && $rs10 && $rs11){
                    // 提交事务
                    Db::commit();
                financelog($orderinfo['buy_id'],$mum,'买入USDT_f1',0);//添加日志
                if($traderMoney>0){
                    financelog($orderinfo['sell_id'],$traderMoney,'交易员利润_f1',0);//添加日志
                }
        ...
        if(request()->isPost()){
            ...
            try{
                ...
                if($rs1 && $rs2 && $rs3 && $rs4 ){
                    // 提交事务
                    Db::commit();
                    financelog($orderinfo['buy_id'],$mum,'买入USDT_f2',0);//添加日志
