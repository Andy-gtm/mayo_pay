...
function getUsdtPrice(){
    $data = curl_get('https://api.coinmarketcap.com/v1/ticker/tether/?convert=CNY');
    $price = 0;
    $data_arr = json_decode($data, true);//dump($data_arr);

    if($data_arr[0]['price_cny']>0){
       $price=round($data_arr[0]['price_cny'],3);
    }
    return $price;
}
...
function financelog($uid,$amount,$note,$status){

     $user=Db::name('merchant')->where('id',$uid)->find();
     $rs = Db::table('think_financelog')->insert(['uid'=>$uid,'user'=>$user['name'], 'note'=>$note, 'amount'=>$amount, 'status'=>$status, 'add_time'=>time()]);
     if($rs){
        return $rs;
     }else{
        return '记录失败';
