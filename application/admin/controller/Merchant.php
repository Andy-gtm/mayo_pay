...
class Merchant extends Base{
                ...
                if($rs1 && $rs2['code'] == 1 && $rsarr[0] && $rsarr[1]){
					...
					Db::table('think_merchant')->where('id', $find['merchant_id'])->setInc('withdraw_amount', $find['num']);
                     writelog(session('adminuid'),session('username'),'用户【'.session('username').'】审核提币:'.$find['merchant_id'].'成功',1);
                      financelog($find['merchant_id'],$find['num'],'提币_1',1);//添加日志
    ...
    public function sssuccessbuy(){
        ...
        try{
            ...
            if($rs1 && $rs2 && $rs3 && $rs4 && $rs5){
                // 提交事务
                Db::commit();
                financelog($orderinfo['sell_id'],($orderinfo['deal_num']+$orderinfo['fee']),'卖出USDT_释放_1',1);//添加日志
                financelog($orderinfo['buy_id'],$mum,'买入USDT_1',0);//添加日志
    ...
    public function ssfail(){
        ...
        try{
            ...
            if($rs1 && $rs2 && $rs3 && $rs4 && $rs5 && $rs6 && $rs7 && $rs8 && $rs9 && $rs10 && $rs11 && $rs22){
                // 提交事务
                Db::commit();
                 financelog($orderinfo['sell_id'],$mum,'卖出USDT_释放',0);//添加日志
                 financelog($orderinfo['buy_id'],$mum,'买入USDT_2',0);//添加日志
                 financelog($orderinfo['sell_id'],$traderMoney,'卖出USDT_交易员利润_2',1);//添加日志
	...
	public function ssfailbuy(){
        ...
        try{
            ...
            if($rs1 && $rs2 && $rs3){
                financelog($orderinfo['sell_id'],($orderinfo['deal_num']+$orderinfo['fee']),'卖出USDT_释放_3',1);//添加日志
                financelog($orderinfo['buy_id'],$mum,'买入USDT_3',0);//添加日志
                // 提交事务
                Db::commit();
                $this->success('操作成功');
            }else{
